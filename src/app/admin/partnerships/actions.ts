...
export async function formPartnership(formData: FormData) {
  ...
  if (!leaderId || !followerId || leaderId === followerId) {
    redirect("/admin/partnerships?error=pick");
  }
  ...
  if (conflict) redirect("/admin/partnerships?error=busy");
  ...
  redirect("/admin/partnerships?ok=formed");
}
...
export async function splitPartnership(formData: FormData) {
  ...
  revalidatePath("/admin/partnerships");
  redirect("/admin/partnerships?ok=split");
}
