...
export async function notifyTransitions() {
  ...
  if (user.role !== "CLUB_MANAGER" || !user.clubId) redirect("/admin");
  if (!mailConfigured()) redirect("/admin?notify=config");

  const list = await upcomingTransitions(user.clubId);
  if (list.length === 0) redirect("/admin");
