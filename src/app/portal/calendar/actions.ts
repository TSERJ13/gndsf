...
function refresh() {
  for (const p of ["/portal/calendar", "/calendar", "/"]) revalidatePath(p);
}
...
export async function createEvent(formData: FormData) {
  ...
  if (!title || !city || !date) redirect("/portal/calendar?error=fields");
  ...
  refresh();
  redirect("/portal/calendar?ok=created");
}
...
export async function deleteEvent(formData: FormData) {
  ...
  refresh();
  redirect("/portal/calendar?ok=deleted");
}
