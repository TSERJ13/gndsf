...
function refresh() {
  for (const p of ["/portal/documents", "/documents"]) revalidatePath(p);
}
...
export async function uploadDocument(formData: FormData) {
  ...
  if (!title || !(file instanceof File) || file.size === 0) {
    redirect("/portal/documents?error=fields");
  }
  if (file.type !== "application/pdf") redirect("/portal/documents?error=type");
  if (file.size > MAX_BYTES) redirect("/portal/documents?error=size");
  if (!process.env.BLOB_READ_WRITE_TOKEN && process.env.VERCEL) {
    redirect("/portal/documents?error=storage");
  }
  ...
  refresh();
  redirect("/portal/documents?ok=uploaded");
}
...
export async function deleteDocument(formData: FormData) {
  ...
  refresh();
  redirect("/portal/documents?ok=deleted");
}
