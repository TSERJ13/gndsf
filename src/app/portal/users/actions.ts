...
export async function createStaffUser(formData: FormData) {
  ...
  if (!name || !email || password.length < 8 || !STAFF_ROLES.includes(role)) {
    redirect("/portal/users?error=fields");
  }
  if (role === "CLUB_MANAGER" && !clubId) redirect("/portal/users?error=club");
  if (await db.user.findUnique({ where: { email } })) {
    redirect("/portal/users?error=exists");
  }
  ...
  redirect("/portal/users?ok=created");
}

export async function toggleUserActive(formData: FormData) {
  const admin = await requireRole(["SUPER_ADMIN"]);
  const id = String(formData.get("id"));
  if (id === admin.id) redirect("/portal/users?error=self");
  ...
  if (target.role === "SUPER_ADMIN" && target.isActive) {
    ...
    if (others === 0) redirect("/portal/users?error=lastadmin");
  }
  ...
  revalidatePath("/portal/users");
  redirect("/portal/users?ok=toggled");
}
...
export async function resetUserPassword(formData: FormData) {
  ...
  const id = String(formData.get("id"));
  const password = String(formData.get("password") ?? "");
  if (password.length < 8) redirect("/portal/users?error=shortpass");
  ...
  revalidatePath("/portal/users");
  redirect("/portal/users?ok=reset");
}
